...
export const GENERIC_ABILITIES: GenericAbilityDef[] = [
  ...
  },
  {
    name: 'Buck',
    actionType: 'reaction',
    descriptionTemplate: 'Trigger A creature Mounts or uses the Command an Animal action while riding the monster. Effect The triggering creature must succeed at a DC {dc} Reflex saving throw or fall off the creature and land Prone. If the save is a critical failure, the triggering creature also takes {damage} in addition to the normal damage for the fall.',
    shortDescriptionTemplate: 'DC {dc} Reflex save, {damage} on critical failure',
    variables: [
      { key: 'dc', label: 'Reflex DC', type: 'dc' },
      { key: 'damage', label: 'Fall Damage', type: 'damage' },
    ],
  },
  {
    name: 'Catch Rock',
  ...
  },
  {
    name: 'Constrict',
    actionType: 'single',
    descriptionTemplate: 'The monster deals {damage} to any number of creatures Grabbed or Restrained by it. Each of those creatures can attempt a DC {dc} basic Fortitude save.',
  ...
  },
  {
    name: 'Engulf',
    actionType: 'two',
    descriptionTemplate: 'The monster Strides up to double its Speed and can move through the spaces of any creatures in its path. Any creature of {size} or smaller whose space the monster moves through can attempt a DC {dc} Reflex save to avoid being engulfed. A creature unable to act automatically critically fails this save. If a creature succeeds at its save, it can choose to be either pushed aside (out of the monster\'s path) or pushed in front of the monster to the end of the monster\'s movement. The monster can attempt to Engulf the same creature only once in a single use of Engulf. The monster can contain as many creatures as can fit in its space. A creature that fails its save is pulled into the monster\'s body. It is Grabbed, is Slowed 1, and has to hold its breath or start suffocating. The creature takes {damage} when first engulfed and at the end of each of its turns while it\'s engulfed. An engulfed creature can get free by Escaping. An engulfed creature can attack the monster engulfing it, but only with unarmed attacks or with weapons of light Bulk or less. The engulfing creature is Off-Guard against the attack. If the monster takes piercing or slashing damage equaling or exceeding {rupture} from a single attack or spell, the engulfed creature cuts itself free.',
    shortDescriptionTemplate: '{size} or smaller, DC {dc} Reflex save, {damage} per turn, Rupture {rupture}',
    variables: [
  ...
  },
  {
    ...
    actionType: 'single',
    descriptionTemplate: 'The monster deals {damage} to any number of creatures Grabbed or Restrained by it. Each of those creatures can attempt a DC {dc} basic Fortitude save. A creature that fails this save falls Unconscious, and a creature that succeeds is then temporarily immune to falling unconscious from Greater Constrict for 1 minute.',
    shortDescriptionTemplate: '{damage}, DC {dc} basic Fortitude save',
    variables: [
      { key: 'damage', label: 'Damage', type: 'damage' },
      { key: 'dc', label: 'Fortitude DC', type: 'dc' },
  ...
  },
  {
    ...
    actionType: 'free',
    descriptionTemplate: 'Trigger The monster\'s last action was a successful Strike that lists Improved Knockdown in its damage entry. Effect The monster attempts to Trip the creature as a free action. This attempt neither applies nor counts toward the monster\'s multiple attack penalty.',
    shortDescriptionTemplate: '',
    variables: [],
  },
  {
  ...
  },
  {
    name: 'Regeneration',
    actionType: 'passive',
    descriptionTemplate: 'This monster regains {hp} Hit Points each round at the beginning of its turn. Its Dying condition never increases beyond Dying 3 as long as its regeneration is active. However, if it takes {suppressedBy} damage, its regeneration deactivates until the end of its next turn. Deactivate the regeneration before applying any damage of a listed type, since that damage might kill the monster by bringing it to Dying 4.',
    shortDescriptionTemplate: '{hp} (suppressed by {suppressedBy})',
    variables: [
      { key: 'hp', label: 'HP per Round', type: 'text', placeholder: 'e.g. 15' },
      { key: 'suppressedBy', label: 'Suppressed By (damage type)', type: 'text', placeholder: 'e.g. fire' },
    ],
  },
  {
    name: 'Rend',
    actionType: 'single',
    descriptionTemplate: 'Requirements The monster hit the same enemy with two consecutive {strikeType} Strikes in the same round. Effect The monster automatically deals that Strike\'s damage again to the enemy.',
  ...
  },
  {
    name: 'Scent',
    actionType: 'passive',
    descriptionTemplate: 'Scent (imprecise) {range} feet. Scent involves sensing creatures or objects by smell. The sense can detect creatures that emit an aroma within the listed range; double or triple the range if the creature emits a heavy aroma or is upwind, and reduce it if downwind.',
  ...
  },
  {
    name: 'Swallow Whole',
    actionType: 'single',
    descriptionTemplate: 'The monster attempts to swallow a creature of {size} or smaller that it has grabbed or restrained in its jaws or mouth. If a swallowed creature is of the maximum size listed, the monster can\'t use Swallow Whole again. The monster attempts a check opposed by the grabbed creature\'s Reflex DC. If it succeeds, it swallows the creature. A swallowed creature is Grabbed, is Slowed 1, and has to hold its breath or start suffocating. The swallowed creature takes {damage} when first swallowed and at the end of each of its turns while it\'s swallowed. A swallowed creature can Escape. If the monster takes piercing or slashing damage equaling or exceeding {rupture} from a single attack or spell, the swallowed creature cuts itself free.',
  ...
  },
  {
    name: 'Throw Rock',
    actionType: 'single',
    descriptionTemplate: 'The monster picks up a rock within reach or retrieves a stowed rock and throws it, making a ranged Strike.',
    shortDescriptionTemplate: '',
    variables: [],
  },
  {
  ...
  },
  {
    name: 'Tremorsense',
    actionType: 'passive',
    descriptionTemplate: 'Tremorsense (imprecise) {range} feet. Tremorsense allows a monster to feel the vibrations through a solid surface caused by movement. Tremorsense functions only if the monster is on the same surface as the subject, and only if the subject is moving along (or burrowing through) the surface.',
    shortDescriptionTemplate: 'imprecise {range} feet',
    variables: [
      { key: 'range', label: 'Range (feet)', type: 'text', placeholder: 'e.g. 30' },
    ],
  },
  {
    name: 'Wavesense',
    actionType: 'passive',
    descriptionTemplate: 'Wavesense (imprecise) {range} feet. This sense allows a monster to feel vibrations caused by movement through a liquid. Wavesense functions only if monster and the subject are in the same body of liquid, and only if the subject is moving through the liquid.',
    shortDescriptionTemplate: 'imprecise {range} feet',
    variables: [
      { key: 'range', label: 'Range (feet)', type: 'text', placeholder: 'e.g. 30' },
    ],
...
export const ABILITY_GLOSSARY: Record<string, string> = {
  ...
  "Improved Grab": "Trigger The monster's last action was a successful Strike that lists Improved Grab in its damage entry, or the monster has a creature Grabbed or Restrained Effect If used after a Strike, the monster attempts to [[/act grapple]] the creature using the body part it attacked with as a free action. This attempt neither applies nor counts toward the creature's multiple attack penalty. The monster can instead use Improved Grab as an action and choose one creature it's grabbing or restraining with an appendage that has Improved Grab to automatically extend that condition to the end of the monster's next turn.",
  "Improved Knockdown": "Trigger The monster's last action was a successful Strike that lists Improved Knockdown in its damage entry Effect The monster attempts to [[/act trip]] the creature as a free action. This attempt neither applies nor counts toward the monster's multiple attack penalty.",
  "Improved Push": "Trigger The monster's last action was a successful Strike that lists Improved Push in its damage entry Effect The monster attempts to [[/act shove]] the creature. This attempt neither applies nor counts toward the monster's multiple attack penalty. If Improved Push lists a distance, change the distance the creature is pushed on a success to that distance.",
  "Knockdown": "Requirements The monster's last action was a successful Strike that lists Knockdown in its damage entry Effect The monster attempts to [[/act trip]] the creature. This attempt neither applies nor counts toward the monster's multiple attack penalty.",
  ...
  "Sneak Attack": "When the monster Strikes a creature that has the Off-Guard condition with an agile or finesse melee weapon, an agile or finesse unarmed attack, or a ranged weapon attack, it also deals the listed precision damage. For a ranged attack with a thrown weapon, that weapon must also be an agile or finesse weapon.",
